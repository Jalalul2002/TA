...
<body class="bg-gray-100 text-gray-900">
    <div class="max-w-6xl mx-auto p-6 bg-white shadow-md rounded-lg">
        <h2 class="text-2xl font-bold text-center mb-4">Laporan Data Perencanaan
            {{ $type == 'bhp' ? 'Bahan Habis Pakai' : 'Inventaris' }}</h2>
        <div class="flex items-center justify-around flex-row mb-4">
            <p class="text-sm text-gray-700">Tahun Perencanaan: <strong>{{ $perencanaan->nama_perencanaan }}</strong></p>
                ...
                <tbody>
                    @foreach ($data as $row)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-3 py-2">{{ $row->product->product_name ?? '-' }}</td>
                            <td class="border border-gray-300 px-3 py-2">{{ $row->product->product_detail ?: '-' }}</td>
                            <td class="border border-gray-300 px-3 py-2">{{ $row->product->merk ?: '-' }}</td>
                            <td class="border border-gray-300 px-3 py-2">{{ $row->product->product_type ?? '-' }}</td>
                            ...
                            <td class="border border-gray-300 px-3 py-2">{{ $row->product->product_unit ?? '-' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
