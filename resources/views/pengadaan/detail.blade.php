<x-app-layout>
    @php
        ...
        $defaultKode = $kodeBarang[$dataRealisasi->prodi] ?? '700';
    @endphp
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ $dataRealisasi->type === 'bhp' ? route('realisasi.bhp') : route('realisasi.inv') }}">
                {{ __('◀️ Detail Pengadaan Barang') }}
            </a>
        </h2>
    </x-slot>
    <form id="complete-form" action="{{ route('realisasi.complete', $dataRealisasi->id) }}" method="POST"
            ...
            <div class="bg-white shadow-lg rounded-xl p-6 mb-6 border border-gray-200">
                <div class="flex flex-col space-y-4">
                    <div class="flex justify-between items-center border-b pb-4">
                        <h1 class="text-2xl font-bold text-gray-900">Detail Pengadaan
                            {{ $dataRealisasi->type === 'bhp' ? 'Barang Habis Pakai' : 'Aset Inventaris' }}</h1>
                        <span
                            class="text-sm font-bold rounded-full px-4 py-2
                            {{ $dataRealisasi->status == 'belum' ? 'bg-red-100 text-red-500' : 'bg-green-100 text-green-500' }}">{{ $dataRealisasi->status == 'belum' ? 'Belum Diselesaikan' : 'Sudah Diselesaikan' }}
                        </span>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div class="flex items-center gap-x-8">
                            <div class="">
                                <h2 class="text-gray-500 text-sm font-medium">Pengadaan</h2>
                                <h1 class="text-lg font-semibold items-baseline text-gray-900">
                                    {{ "{$dataRealisasi->name} - Prodi {$dataRealisasi->prodi}" }}
                                </h1>
                            </div>
                            @if ($dataRealisasi->status === 'belum' && Auth::user()->usertype !== 'user')
                                <button title="Edit Pengadaan" @click="openEdit = true"
                        ...
                        <div>
                            ...
                            <h1 class="text-lg font-semibold text-gray-900">
                                {{ ($dataRealisasi->latestUpdater?->updater?->name ?? $dataRealisasi->updater->name) . ' | ' . ($dataRealisasi->latestUpdater?->updated_at->format('d M Y H:i') ?? $dataRealisasi->updated_at->format('d M Y H:i')) }}
                            </h1>
                        </div>
                        <div>
                                ...
                                <div class="flex justify-end flex-row items-center">
                                    <div>
                                        <button
                                            ...
                                            <span>Selesaikan Pengadaan</span>
                                        </button>
                                    </div>
                                </div>
                            @else
                                <div>
                                    ...
                                    <h1 class="text-lg font-semibold text-gray-900">
                                        {{ $dataRealisasi->updater->name . ' | ' . $dataRealisasi->updated_at->format('d M Y H:i') }}
                                    </h1>
                                </div>
                            @endif
                        @endif
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-6">
                    <div
                        ...
                        <div class="flex items-center space-x-1 xl:space-x-2 justify-end">
                            ...
                            @endif
                            <a href="{{ route('realisasi.export.id', $dataRealisasi->id) }}">
                                <div
                            ...
                            <a href="{{ route('realisasi.print.id', $dataRealisasi->id) }}" target="_blank">
                                <div
                                    ...
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        ...
                                        @if ($dataRealisasi->status === 'belum' && Auth::user()->usertype !== 'user')
                                            <td class="py-2 flex flex-row gap-x-2 justify-center">
                                                <a title="Edit Data"
                                                    href="{{ route('realisasi.edit', $product->id) }}">
                                                    <div
                                                ...
                                                <form action="{{ route('realisasi.destroy.item', $product->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                    ...
                    <div
                        ...
                        <div x-show="openEdit" x-cloak
                            ...
                            <div class="mt-3">
                                ...
                                <div>
                                    <form action="{{ route('realisasi.update-detail', $dataRealisasi->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="mt-4">
                        ...
                        <div x-show="openModal" x-cloak
                            ...
                            <div x-data="{ selectedProduct: '', stock: 0, satuan: '', purchase_price: 0, total_price: 0, showNewProductForm: false }">
                                <div class="mt-3" x-data="priceCalculator()">
                                    ...
                                    <div>
                                        <form action="{{ route('realisasi.store.item', $dataRealisasi->id) }}"
                                            method="POST">
                                            @csrf
                                            <div class="mt-4">
                                            ...
                                            <div x-show="showNewProductForm" class="mt-4 bg-gray-100 p-4 rounded-md">
                                                ...
                                                <div class="mt-2">
                                                    ...
                                                    <input type="text" name="new_location" id="new_location"
                                                        value="{{ $dataRealisasi->prodi }}" readonly
                                                        class="mt-1 block w-full p-2.5 py-2 text-sm border-gray-300 rounded-md">
                                                </div>
                                            </div>
                                            <div class="mt-4">
                                            ...
                                            <div class="mt-4 flex flex-row gap-x-2">
                                                ...
                                                <div class="w-full">
                                                    <label for="quantity"
                                                        class="block text-sm font-medium text-gray-700">Banyaknya</label>
