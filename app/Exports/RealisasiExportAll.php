...
class RealisasiExportAll implements FromCollection, WithHeadings, WithMapping, WithCustomStartCell, WithEvents
{
    /**
    ...
    {
        $query = DataRealisasi::with(['items', 'items.product'])->where('type', $this->type);
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $title = "Data Pengadaan {$typeText} Prodi {$locationText}";
    ...
    public function map($pengadaan): array
    {
        ...
        foreach ($pengadaan->items as $item) {
            $rows[] = [
                ...
                $pengadaan->stock > 0 ? $pengadaan->stock : '0',
