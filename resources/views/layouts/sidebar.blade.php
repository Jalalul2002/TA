<aside id="logo-sidebar"
    ...
    @php
        $menus = [
            [
                'route' => Auth::user()->usertype == 'admin' ? route('admin.dashboard') : route('dashboard'),
                'isActive' => Request::routeIs(['dashboard', 'admin.dashboard']),
                'icon' =>
                    '<div class="p-2 bg-uinGreen group-hover:bg-uinBlue rounded-lg"><svg class="size-4 fill-white" xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="512" height="512"><path d="M7,0H4A4,4,0,0,0,0,4V7a4,4,0,0,0,4,4H7a4,4,0,0,0,4-4V4A4,4,0,0,0,7,0ZM9,7A2,2,0,0,1,7,9H4A2,2,0,0,1,2,7V4A2,2,0,0,1,4,2H7A2,2,0,0,1,9,4Z"/><path d="M20,0H17a4,4,0,0,0-4,4V7a4,4,0,0,0,4,4h3a4,4,0,0,0,4-4V4A4,4,0,0,0,20,0Zm2,7a2,2,0,0,1-2,2H17a2,2,0,0,1-2-2V4a2,2,0,0,1,2-2h3a2,2,0,0,1,2,2Z"/><path d="M7,13H4a4,4,0,0,0-4,4v3a4,4,0,0,0,4,4H7a4,4,0,0,0,4-4V17A4,4,0,0,0,7,13Zm2,7a2,2,0,0,1-2,2H4a2,2,0,0,1-2-2V17a2,2,0,0,1,2-2H7a2,2,0,0,1,2,2Z"/><path d="M20,13H17a4,4,0,0,0-4,4v3a4,4,0,0,0,4,4h3a4,4,0,0,0,4-4V17A4,4,0,0,0,20,13Zm2,7a2,2,0,0,1-2,2H17a2,2,0,0,1-2-2V17a2,2,0,0,1,2-2h3a2,2,0,0,1,2,2Z"/></svg></div>',
                'label' => 'Dashboard',
            ],
            [
                'route' => '/data-aset',
        ...
        <ul class="space-y-2 font-normal">
                ...
                @else
                    <li x-data="{ open: false }">
                        <button @click="open = !open"
                            class="flex items-center w-full px-3 py-4 text-base transition duration-300 rounded-lg group {{ $menu['isActive'] ? 'bg-uinOrange text-white' : 'bg-white text-gray-900 hover:bg-uinYellow hover:text-uinBlue' }}">
                            {!! str_replace('bg-uinGreen', $menu['isActive'] ? 'bg-uinBlue' : 'bg-uinGreen', $menu['icon']) !!}
                            <span
                                class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap hover:text-white">{{ $menu['label'] }}</span>
                            <svg class="w-3 h-3 transition-transform duration-300 {{ $menu['isActive'] ? 'text-uinBlue' : 'text-gray-900 hover:text-uinBlue' }}"
            ...
            <li>
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <a href="{{ route('logout') }}" class="flex items-center px-3 py-4 text-uinRed rounded-lg hover:bg-uinRed hover:font-medium hover:text-white group"
                        onclick="event.preventDefault();
                                    this.closest('form').submit();">
                        <div class="p-2 rounded-lg bg-uinRed group-hover:bg-white">
